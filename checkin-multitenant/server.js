...
// 题目结构清洗：只保留安全字段，生成稳定 id
function sanitizeQuestions(arr) {
    if (!Array.isArray(arr)) return [];
    return arr
        .filter(q => q && (q.label || '').toString().trim())
        .map((q, i) => {
            const type = q.type === 'choice' ? 'choice' : 'text';
            const label = String(q.label).trim().slice(0, 50);
            const required = !!q.required;
            const options = type === 'choice'
                ? (Array.isArray(q.options) ? q.options.map(o => String(o).trim()).filter(Boolean).slice(0, 20) : [])
                : [];
            const id = (q.id && typeof q.id === 'string') ? q.id.slice(0, 40) : ('q' + (i + 1) + '_' + genId().slice(0, 4));
            return { id, type, label, required, options };
        });
}
...
function sanitizeSettings(s) {
    s = s || {};
    return {
        validateRoster: !!s.validateRoster,   // 是否启用名单校验（仅名单内可签到/报名）
        allowWalkin: s.allowWalkin === false ? false : true // 名单校验开启时，是否允许非名单人员自主报名（默认允许并标记）
    };
}

// 从提交中过滤出合法答案（仅保留本主题已定义题目的答案）
function pickAnswers(questions, raw) {
    const out = {};
    if (!questions || !questions.length || !raw || typeof raw !== 'object') return out;
    questions.forEach(q => {
        const v = raw[q.id];
        if (v === undefined || v === null || v === '') return;
        out[q.id] = String(v).slice(0, 500);
    });
    return out;
}
...
// 主题公开信息：参与者页据此加载活动名称、配色、报名题目与名单校验开关
app.get('/api/theme', (req, res) => {
    ...
    res.json({
        ...
        theme: {
            ...
            validateRoster: !!(theme.settings && theme.settings.validateRoster),
            allowWalkin: !(theme.settings && theme.settings.allowWalkin === false)
        }
    });
});

// 查询手机号状态（带出名单中的姓名单位，便于自动补全）
app.post('/api/lookup', (req, res) => {
    ...
    if (at) return res.json({ status: 'attendee', user: { name: at.name, phone: at.phone, unit: at.unit || '--' }, fromRoster: true });
    res.json({ status: 'new', phone: clean });
});

// 名单内签到（支持报名题目答案、名单校验）
app.post('/api/checkin', (req, res) => {
    const { code, name, phone, unit, answers } = req.body;
    const clean = getPhone(phone);
    if (!code || !clean || !isValidPhone(clean)) return res.status(400).json({ success: false, message: '参数错误' });
    const data = loadData();
    const theme = findTheme(data, code);
    ...
    // 名单校验：开启了且不允许非名单人员 -> 不在名单拒绝
    const validateRoster = !!(theme.settings && theme.settings.validateRoster);
    const allowWalkin = !(theme.settings && theme.settings.allowWalkin === false);
    const inRoster = theme.attendees.some(a => a.phone === clean);
    if (validateRoster && !inRoster && !allowWalkin) {
        return res.status(400).json({ success: false, message: '您不在本次活动的报名名单中，无法签到' });
    }
    ...
    const rosterUser = theme.attendees.find(a => a.phone === clean);
    const record = {
        id: genId(),
        name: (name || (rosterUser && rosterUser.name) || '--').toString().slice(0, 30),
        phone: clean,
        unit: (unit || (rosterUser && rosterUser.unit) || '--').toString().slice(0, 50),
        checkinTime: Date.now(),
        fromRoster: !!rosterUser,
        answers: pickAnswers(theme.questions, answers)
    };
...
app.post('/api/signup', (req, res) => {
    ...
    if (validateRoster && inRoster) {
        return res.status(400).json({ success: false, message: '您已在报名名单中，请使用签到功能' });
    }
    if (validateRoster && !inRoster && !allowWalkin) {
        return res.status(400).json({ success: false, message: '您不在报名名单中，无法报名' });
    }
    ...
    const ans = pickAnswers(theme.questions, answers);
    if (existing) {
        existing.checkinTime = Date.now();
        existing.name = name.trim();
        existing.unit = unit.trim();
        existing.answers = ans;
        saveData(data);
...
app.post('/api/admin/login', (req, res) => {
    ...
    const token = makeToken({ u: username, scope: 'admin', e: Date.now() + TOKEN_TTL });
    res.json({ success: true, token, isSuper: !!admin.isSuper, username: admin.username });
});

// 管理操作鉴权：校验 Token（管理员身份），主题在请求体 code 中指定
function canManageTheme(admin, theme) { return admin.isSuper || (theme && theme.owner === admin.username); }
...
function adminAuth(req, res, next) {
    ...
    if (!admin) return res.status(403).json({ success: false, message: '管理员不存在' });
    req.ctx = { data, admin };
    next();
}

// 按 code 取主题并校验权限（管理接口通用）
function resolveTheme(req, res) {
    const code = (req.body && req.body.code || '').trim();
    if (!code) { res.status(400).json({ success: false, message: '请指定主题编码' }); return null; }
    const { data, admin } = req.ctx;
    const theme = findTheme(data, code);
    if (!theme || !canManageTheme(admin, theme)) { res.status(403).json({ success: false, message: '无权限或主题不存在' }); return null; }
    return theme;
}

// 管理员可管理的主题列表（超级管理员看全部）
app.post('/api/admin/themes', adminAuth, (req, res) => {
    const { data, admin } = req.ctx;
    const list = data.themes
        .filter(t => canManageTheme(admin, t))
        .map(t => ({ code: t.code, name: t.name, color: t.color || '#4F46E5', owner: t.owner, attendees: (t.attendees || []).length, checkins: (t.checkins || []).length, signups: (t.signups || []).length }));
    res.json({ success: true, themes: list });
});

// 管理员创建会议（主题）—— 一个管理员可创建多个，编码全局唯一
app.post('/api/admin/theme/create', adminAuth, (req, res) => {
    const { code, name, color, questions, settings } = req.body;
    ...
    data.themes.push({
        code: c, name: name.trim(), color: color || '#4F46E5', owner: admin.username, createdAt: Date.now(),
        attendees: [], checkins: [], signups: [],
        questions: sanitizeQuestions(questions),
        settings: sanitizeSettings(settings)
    });
    saveData(data);
    res.json({ success: true, message: `会议「${name.trim()}」创建成功`, code: c });
});

// 管理员删除自己创建的主题
app.post('/api/admin/theme/delete', adminAuth, (req, res) => {
    const { data, admin } = req.ctx;
    const c = (req.body.code || '').trim();
    const theme = findTheme(data, c);
    if (!theme || !canManageTheme(admin, theme)) return res.status(403).json({ success: false, message: '无权限或主题不存在' });
    data.themes = data.themes.filter(t => t.code !== c);
    saveData(data);
    res.json({ success: true, message: `主题「${c}」已删除` });
});

// 管理员配置主题的报名题目与名单校验策略
app.post('/api/admin/theme/config', adminAuth, (req, res) => {
    const { data, admin } = req.ctx;
    const theme = resolveTheme(req, res);
    if (!theme) return;
    const { name, color, questions, settings } = req.body;
    if (name && name.trim()) theme.name = name.trim().slice(0, 50);
    if (color) theme.color = color;
    if (questions !== undefined) theme.questions = sanitizeQuestions(questions);
    if (settings !== undefined) theme.settings = sanitizeSettings(settings);
    saveData(data);
    res.json({ success: true, message: '主题设置已保存', theme: { code: theme.code, name: theme.name, questions: theme.questions, settings: theme.settings } });
});
...
app.post('/api/admin/data', adminAuth, (req, res) => {
    ...
    const theme = resolveTheme(req, res);
    if (!theme) return;
    res.json({ success: true, data: { code: theme.code, name: theme.name, color: theme.color, attendees: theme.attendees, checkins: theme.checkins, signups: theme.signups, questions: theme.questions || [], settings: theme.settings || {} } });
});

app.post('/api/admin/import', adminAuth, (req, res) => {
    const data = req.ctx.data;
    const theme = resolveTheme(req, res);
    if (!theme) return;
    ...
    lines.forEach((line, idx) => {
        ...
        // 自动跳过表头行（姓名,电话,单位 / name,phone,unit 等）
        const first = (parts[0] || '').toLowerCase();
        if (['姓名', 'name', '手机号', 'phone', '电话'].includes(first)) return;
...
app.post('/api/admin/checkin', adminAuth, (req, res) => {
    ...
    if (theme.attendees.some(a => a.phone === clean)) {
        const roster = theme.attendees.find(a => a.phone === clean);
        theme.checkins.push({ id: genId(), name: name || roster.name, phone: clean, unit: (unit || roster.unit || '--'), checkinTime: Date.now(), fromRoster: true, answers: {} });
        saveData(data);
        return res.json({ success: true, message: `${name || roster.name} 签到成功` });
    }
    const ex = theme.signups.find(s => s.phone === clean);
    if (ex) {
        ex.checkinTime = Date.now();
        saveData(data);
        return res.json({ success: true, message: `${name || ex.name} 签到成功` });
    }
    theme.signups.push({ id: genId(), name: name || '--', phone: clean, unit: unit || '--', signupTime: Date.now(), checkinTime: Date.now(), fromRoster: false, answers: {} });
    saveData(data);
...
app.post('/api/admin/export', adminAuth, (req, res) => {
    ...
    theme.attendees.forEach(a => {
        ...
        const rec = ck || a;
        all.push({ name: a.name, phone: a.phone, unit: a.unit || '--', source: '报名名单', checked: ck ? '已签到' : '未签到', checkinTime: ck ? formatTime(ck.checkinTime) : '--', signupTime: '--', fromRoster: true, answers: (ck && ck.answers) || {} });
    });
    theme.signups.forEach(s => {
        all.push({ name: s.name, phone: s.phone, unit: s.unit || '--', source: '新报名', checked: s.checkinTime ? '已签到' : '未签到', checkinTime: s.checkinTime ? formatTime(s.checkinTime) : '--', signupTime: s.signupTime ? formatTime(s.signupTime) : '--', fromRoster: false, answers: s.answers || {} });
    });
    const headers = ['姓名', '电话', '单位', '来源', '签到状态', '报名时间', '签到时间', ...questions.map(q => q.label)];
    const rows = all.map(p => [
        p.name, p.phone, p.unit, p.source, p.checked, p.signupTime, p.checkinTime,
        ...questions.map(q => (p.answers && p.answers[q.id]) || '')
    ]);
...
app.post('/api/console/themes', consoleAuth, (req, res) => {
    const { data } = req.ctx;
    res.json({ success: true, themes: data.themes.map(t => ({ code: t.code, name: t.name, color: t.color, owner: t.owner, attendees: (t.attendees || []).length, checkins: (t.checkins || []).length, questions: t.questions || [], settings: t.settings || {} })) });
});
...
// 超级管理员配置任意主题的报名题目与名单校验策略
app.post('/api/console/theme/config', consoleAuth, (req, res) => {
    const { code, name, color, questions, settings } = req.body;
    if (!code || !code.trim()) return res.status(400).json({ success: false, message: '主题编码必填' });
    const { data } = req.ctx;
    const theme = findTheme(data, code.trim());
    if (!theme) return res.status(404).json({ success: false, message: '主题不存在' });
    if (name && name.trim()) theme.name = name.trim().slice(0, 50);
    if (color) theme.color = color;
    if (questions !== undefined) theme.questions = sanitizeQuestions(questions);
...
// 开发者总后台：查看所有主题的完整数据（含题目答案）
app.post('/api/console/data/all', consoleAuth, (req, res) => {
    const { data } = req.ctx;
    res.json({ success: true, themes: data.themes.map(t => ({ ...t, questions: t.questions || [], settings: t.settings || {} })) });
});
